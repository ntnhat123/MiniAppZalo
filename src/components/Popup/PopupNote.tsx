        ...
        <div className="bg-white rounded-lg shadow-lg w-full  max-w-4xl">
            ...
            <div className="min-w-full">
              ...
              <div className="mb-4 flex flex-col md:flex-row md:items-center">
                ...
                <input 
                  ...
                  type="text" 
                  readOnly 
                  //value={list.ThoiGianGhiNhatKy} 
                />
              </div>
              {/* Ngày sự kiện/Nhật ký */}
              <div className="mb-4 flex flex-col md:flex-row md:items-start"> {/* Thay đổi md:items-center thành md:items-start */}
          ...
          <div className="md:w-2/3">
            <input 
              ...
              type="text" 
              //value={list.NgaySuKienNhatKy} 
            />
            <small className="text-gray-500 block mt-1">(Nếu trực hôm qua mà hôm nay mới ghi nhật ký thì điền ngày đã trực)</small> {/* Thêm block và margin */}
          </div>
        </div>
              {/* Nhật ký */}
              <div className="mb-4 flex flex-col md:flex-row md:items-center">
              ...
              <div className="mb-4 flex flex-col md:flex-row md:items-center">
                ...
                <select 
                  className="shadow appearance-none border rounded w-full md:w-2/3 py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                  <option>--Tất cả nhiệm vụ--</option>
                    {tasks.map((task, index) => (
                        <option key={index}>{task.DanhMucNhiemVuName}</option>
                    ))}
                </select>
              </div>
              {/* Tình trạng */}
                ...
                <select 
                  ...
                  <option>--Tất cả tình trạng--</option>
                    {statusData.map((status, index) => (
                        <option key={index}>{status.StatusName}</option>
                    ))}
                </select>
              </div>
              {/* File */}
